...
export default function App() {
  ...
  // --- 2. FLAWLESS TIMER ---
  // This interval ONLY cares about time, preventing the keystroke stutter bug.
  useEffect(() => {
    let interval;
    if (status === 'running') {
  ...
  // Calculate live WPM purely mathematically on render
  ...
  useEffect(() => {
    if (mode === 'time' && status === 'running' && timeLeft <= 0) endTest();
  }, [timeLeft, status, mode, endTest]);

  useEffect(() => {
    if (mode === 'words' && status === 'running' && activeWordIndex >= wordSetting) endTest();
  }, [activeWordIndex, wordSetting, status, mode, endTest]);

  // --- 4. PRECISION SCROLLING ---
  // No CSS smooth scrolling. Hard math offsets to prevent bouncing.
  useEffect(() => {
    if (activeWordRef.current && containerRef.current) {
      const parent = containerRef.current;
      const child = activeWordRef.current;
      
      const lineHeight = 46; // Approx pixel height of one line
      if (child.offsetTop > parent.scrollTop + lineHeight * 1.5) {
  ...
  return (
    <div className="app-container">
      <header>
        <div className="logo">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 7h16M4 12h16M4 17h16"/></svg>
          TypeForge
        </div>
        <div className="theme-picker">
          {THEMES.map(t => (
            <div key={t.id} className={`swatch ${theme === t.id ? 'active' : ''}`} style={{ background: t.color }} onClick={() => setTheme(t.id)} title={t.id} />
          ))}
        </div>
      </header>
        ...
        {status !== 'finished' ? (
          <div className="type-card">
            ...
            <div className={`live-stats ${status === 'running' ? 'show' : ''}`}>
              <div className="timer">{mode === 'time' ? timeLeft : timeElapsed}s</div>
              <div className="live-wpm">{liveWpm} wpm</div>
            </div>

            <div className="words-viewport" ref={containerRef} onClick={() => inputRef.current?.focus()}>
              <div className={`focus-overlay ${!isFocused && status !== 'finished' ? 'show' : ''}`}>
                <span><kbd>Click</kbd> or press any key to focus</span>
              </div>
                ...
                {words.map((word, wIdx) => {
                  ...
                  return (
                    <div key={wIdx} ref={isActiveWord ? activeWordRef : null} className={`word ${isActiveWord ? 'active' : ''} ${isPastWord && typed !== word ? 'error-word' : ''}`}>
                      ...
                      {extraLetters.split('').map((char, cIdx) => (
                        <span key={`extra-${cIdx}`} className={`letter extra ${isActiveWord && cIdx === extraLetters.length - 1 ? 'caret-after' : ''}`}>{char}</span>
                      ))}
            ...
            <div className="footer-actions">
              <button onClick={generateTest} className="restart-btn" tabIndex="-1">↻ Restart Test</button>
              <div className="shortcut-hint"><kbd>Tab</kbd> to restart</div>
            </div>
          </div>
        ) : (
          <div className="results-panel show">
            <div className="stats-grid">
              ...
              <div className="stat-box">
                ...
                <div className="stat-value-small">{mode === 'time' ? timeSetting : timeElapsed}s</div>
              </div>
              <div className="stat-box">
                <div className="stat-label">CHARACTERS</div>
                <div className="stat-value-small chars">{rawStats.correct}/{rawStats.incorrect}/{rawStats.extra}/{rawStats.missed}</div>
              </div>
            </div>
                  ...
                  <tbody>
                    {leaderboard.length > 0 ? leaderboard.map((entry, idx) => (
                      <tr key={entry.id}>
                        ...
                        <td className="text-accent">{entry.top_score}</td>
